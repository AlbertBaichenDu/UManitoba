...
#does child voc predict AWC?

CVCpredict<-lmer(AWC ~ Child_Voc_Count + (1|Child_ChildID), data=ADEX_Smartphone_data_excl_nonaps)
anova(CVCpredict)

#Child Voc Scatterplot
CVCscatterplot <- ggplot(data = ADEX_Smartphone_data_excl_nonaps, aes(x = Child_Voc_Count, y = duration, color = ChildAgeGroup)) +
  geom_point() +
